...
| **0:16–0:34** | "This Overview tab isn't decoration — every number here comes from a real database and real on-chain state, and it moves as I use the app. Watch that claims counter — I'll come back to it after a real claim settles." | **Overview** tab → point at the four KPI cards and the two charts. |
...
| **3:10–3:20** | "The compliance SDK is on npm today — `npm install confidia-sdk` — it's linked right in the app's Docs tab. We're explicit about what's real, secrets rotate in production, and the whole thing reproduces from one command. Confidia — private USD distribution, with ZK that actually verifies on-chain." | Quick cut to **Docs** tab → the `confidia-sdk` panel. Then end card with **confidia.vercel.app**, **npmjs.com/package/confidia-sdk**, and the GitHub URL. |
...
- The shared demo vault currently holds real testnet XLM and loses 1 per Happy Path run — plenty of headroom for a take or two, but don't run it dozens of times before recording.
